...
public class Player : MonoBehaviour, ITurnOverInvoker
{
        ...
        Configuration boardConfiguration)
    {
        ...
        while (nodeList.Count > 0)
        {
            ...
            foreach (Configuration child in children)
            {
                ...
                int childDepth = ChildDepth(childNode);
    ...
    int ChildDepth(MinimaxTreeNode<Configuration> childNode)
    {
        if (childNode.Parent != null)
        {
            return 1 + ChildDepth(childNode.Parent);
        }
        else
        {
            return 0;
        ...
        bool maximizing)
    {
        ...
        else
        {
            ...
            float sumChildMinimaxScore = 0;
            if (node.Value.NumBears > 0)
            {
                IList<MinimaxTreeNode<Configuration>> children = node.Children;
                foreach (MinimaxTreeNode<Configuration> child in children)
                {
                    sumChildMinimaxScore += child.MinimaxScore;
                }
                float avgChildMinimaxScore = sumChildMinimaxScore / node.Children.Count;
                node.MinimaxScore = avgChildMinimaxScore;
            }
            else
            {
